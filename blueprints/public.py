...
def _get_common_data(category_filter=None):
    ...
    model_type_filter = request.args.get('model_type', '').strip()
    ...
    if model_type_filter:
        query = query.filter(Image.model_type == model_type_filter)
    ...
    # 构建模型筛选列表
    model_types_query = db.session.query(Image.model_type).filter(
        Image.status == 'approved',
        Image.model_type.isnot(None),
        Image.model_type != ''
    )

    if category_filter:
        model_types_query = model_types_query.filter(Image.category == category_filter)

    if not show_sensitive:
        model_types_query = model_types_query.filter(~Image.tags.any(Tag.is_sensitive == True))

    model_types = [row[0] for row in model_types_query.distinct().order_by(Image.model_type).all()]

    # 构建标签筛选列表；选择模型后，只展示该模型下可用标签
    ...
    if model_type_filter:
        tags_query = tags_query.filter(Image.model_type == model_type_filter)
    ...
    return {
        ...
        'active_model_type': model_type_filter,
    ...
    response_payload = {
        ...
        'meta': {
            ...
            'next_url': url_for(request.endpoint, page=pagination.next_num, per_page=per_page, q=search_query,
                                model_type=model_type_filter, tag=tag_filter, sort=sort_by,
